...
function editExpense(event) {
    if (event.target.classList.contains("edit-btn")) {
        ...
        // Prompt the user to enter the new name and amount for the expense
        const newName = prompt("Enter the new name for the expense:", expenseToEdit.name);
        const newAmount = prompt("Enter the new amount for the expense:", expenseToEdit.amount);

        // Validate the new name and amount
        if (newName !== null && newAmount !== null && !isNaN(parseFloat(newAmount))) {
            // Update the expense object
            expenseToEdit.name = newName;
            expenseToEdit.amount = parseFloat(newAmount);

            // Update the expenses array
            expenses[expenseIndex] = expenseToEdit;
